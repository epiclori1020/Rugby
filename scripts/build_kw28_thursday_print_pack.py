...
OUT_DIR = ROOT / "print_pdfs" / "DONNERSTAG_2026-07-09_DRUCKEN"
...
TRAINING_SRC = ROOT / "plans" / "offseason_coach_sheets" / "KW28_thursday_training_compact_2026-07-09.md"
DEEP_SRC = ROOT / "docs" / "22_kw28_thursday_deep_playbook_2026-07-09.styled.pdf"
...
def checkin_story(styles: dict[str, ParagraphStyle]) -> list:
    story = []
    story.append(para("Check-in | Donnerstag 09.07.2026", styles["title"]))
    story.append(
        para(
    ...
    story.append(
        para(
            "3A angepasst: Dienstag war nachgeholte 2B. Keine Nachhol-Spitze. Kein Full Contact. "
            "Kopf/Nacken/Schwindel/neurologisch = Rot.",
            styles["red"],
        )
    )
    header = ["Nr", "Name", "Pos", "Cluster", "R", "Di-Reaktion / Beine", "Schmerz + Ort", "Ret", "A", "Limit heute"]
    ...
    story.append(
        para(
            "Startentscheidung: Gruen = angepasster 3A-Plan. Gelb = Contact Prep reduzieren, Kraft 2 Saetze, Conditioning streichen. Rot = kein normales Training.",
            styles["meta"],
        )
    )
    return story
...
def observation_story(styles: dict[str, ParagraphStyle]) -> list:
    story = []
    story.append(para("Beobachtung + Nachbereitung | Donnerstag 09.07.2026", styles["title"]))
    story.append(
        para(
            "Nur Auffaelligkeiten eintragen. Ziel: Dienstag 14.07 sauber entscheiden und keine Lastspitze aus der Nachholung erzeugen.",
            styles["meta"],
        )
    )
    story.append(
        para(
            "Kuerzen: zuerst Conditioning, dann Contact Prep, dann Kraft auf 2 Saetze. Speed nur als kurze Qualitaet behalten.",
            styles["red"],
        )
    )

    story.append(para("Beobachtung pro Spieler", styles["section"]))
    header = ["Nr", "Name", "Speed / Prep", "Power", "Kraft Pods", "Contact / Microdose", "sRPE", "Pain/Issue / 14.07"]
    rows = [[str(i), "", "", "", "", "", "", ""] for i in range(1, 21)]
    story.append(
        make_table(
            [[para(c, styles["cell_bold"]) for c in header]]
            + [[para(c, styles["cell"]) for c in row] for row in rows],
            [8 * mm, 33 * mm, 41 * mm, 34 * mm, 42 * mm, 48 * mm, 14 * mm, 47 * mm],
            [6.0 * mm] + [7.0 * mm] * 20,
        )
    )
    story.append(Spacer(1, 3))
    story.append(
        para(
            "Coach-Nachbereitung: Anwesend __ | G/Y/R __/__/__ | Contact nein __ | "
            "Kraft 2/3 Saetze __ | Conditioning 0/1/2 Serien __ | Dienstag 14.07 reduzieren/Ruecksprache: ____________________",
            styles["meta"],
        )
    )
    return story
...
def build_checkin_pdfs() -> None:
    styles = make_styles()
    make_doc(OUT_COMBINED, "KW28 Donnerstag Check-in Beobachtung 2 Seiten 2026-07-09").build(
        checkin_story(styles) + [PageBreak()] + observation_story(styles)
    )
    make_doc(OUT_CHECKIN, "KW28 Donnerstag Check-in 2026-07-09").build(checkin_story(styles))
    make_doc(OUT_OBSERVATION, "KW28 Donnerstag Beobachtung Nachbereitung 2026-07-09").build(observation_story(styles))
