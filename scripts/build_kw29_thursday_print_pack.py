...
def checkin_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    story.append(
        para(
            "4B-Bruecke: Dienstag war nachgeholte 3B. Keine Nachhol-4A. Kein Fly bei Gelb/Returner. "
    ...
    story.append(
        para(
            "Startentscheidung: Gruen = normaler Bruecken-Plan. Gelb = kein Fly/Decel, Kraft 2 Saetze, Tempo streichen. Rot = kein normales Training.",
            styles["meta"],
        )
    )
    return story
...
def observation_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    story.append(
        para(
            "Nur Auffaelligkeiten eintragen. Ziel: KW30 sauber entscheiden und keine Lastspitze aus der Verschiebung erzeugen.",
            styles["meta"],
        )
    )
    story.append(
        para(
            "Kuerzen: zuerst Tempo, dann Cluster-Robustheit, dann Kraft auf 2 Saetze. Speed nur als kurze Qualitaet behalten.",
            styles["red"],
        )
    )

    story.append(para("Beobachtung pro Spieler", styles["section"]))
    header = ["Nr", "Name", "Track Prep", "Speed Quality", "Kraftsignal", "Cluster / Tempo", "sRPE", "Pain/Issue / KW30"]
    rows = [[str(i), "", "", "", "", "", "", ""] for i in range(1, 21)]
    story.append(
        make_table(
            [[para(c, styles["cell_bold"]) for c in header]]
            + [[para(c, styles["cell"]) for c in row] for row in rows],
            [8 * mm, 33 * mm, 41 * mm, 40 * mm, 42 * mm, 42 * mm, 14 * mm, 47 * mm],
            [6.0 * mm] + [7.0 * mm] * 20,
        )
    )
    story.append(Spacer(1, 3))
    story.append(
        para(
            "Coach-Nachbereitung: Anwesend __ | G/Y/R __/__/__ | Fly nein __ | Decel nein __ | "
            "Kraft 2/3 Saetze __ | Tempo 0/Run/Bike __ | KW30 reduzieren/Ruecksprache: ____________________",
            styles["meta"],
        )
    )
    return story
