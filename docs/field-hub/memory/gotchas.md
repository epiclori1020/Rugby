...
| active | Beta-E2E darf keinen vorbefuellten Produktzustand voraussetzen oder bestehende Produktdaten umschreiben. | Nur eindeutig markierte synthetische Fixtures verwenden, eigene IDs verfolgen, den gewaehlten Session-Kontext abdecken, Cleanup in `finally` ausfuehren und die Remote-Abwesenheit danach erneut abfragen; Restdaten halten das Gate rot. |
| active | Authentifizierte Browser-QA kann Zugangsdaten oder personenbezogene Screens persistieren oder an falsche Ziele senden. | Auth-Screenshots hart blockieren, Remote-Ziele nur per exakter HTTPS-Origin erlauben, URL-Credentials ablehnen und QA-Credentials vor dem Start lokaler Preview-Child-Prozesse aus deren Environment entfernen. |
