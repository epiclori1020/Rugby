...
| 2026-07-09 | Save-Feedback in OnField Coach muss ehrlich zwischen gespeichert, Sync offen, offline lokal und nicht gespeichert unterscheiden. | Redesign-v2 R4 verhindert stumme Saves und rohe Backend-/Env-/RLS-Copy. Operative Screens nutzen gemeinsame Meldungen wie `gespeichert`, `wartet auf Sync`, `offline lokal gespeichert` und `nicht gespeichert - erneut versuchen`; Auth/Public/Kiosk mappen Fehler auf coachnahe Trust-Copy. | Aktiv |
| 2026-07-10 | `Squad heute` trennt erwarteten Kader von heutiger Anwesenheit: operativer Kader = erwartete aktive Spieler plus unerwartet Anwesende; bei fehlender Erwartungsbasis gilt die aktive Spielerliste als First-run-Fallback. Ampel-, Returner- und Aufmerksamkeitslogik gilt nur fuer eingecheckte Spieler. | Nicht eingecheckte Spieler bleiben eine Check-in-Aufgabe und duerfen weder als heutige Warnung noch als Returner erscheinen. Der First-run-Fallback verhindert einen irrefuehrenden Null-Kader, solange noch keine historische Erwartungsbasis existiert. Dadurch bleiben `Anwesend <= Kader` und die Tageslage coachfachlich wahr. | Aktiv |
...
| 2026-07-14 | Analyse-Trends werden in Redesign-v2 R10 als leichte native React/SVG-Diagramme gebaut; die exakten Ergebnislisten bleiben als zugaengliche Datenreferenz erhalten. | Vor der externen Beta ist der Wochenverlauf visuell schneller lesbar, ohne eine schwere Chart-Library, neue Datenarchitektur oder color-only Semantik einzufuehren. | Aktiv |
