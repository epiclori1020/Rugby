...
| Check-in | Sprint 9 ist umgesetzt; Redesign-v2 R7A hat den Roster am 2026-07-10 auf die gemeinsame v2-`AthleteRow` mit Readiness-Dot, Traffic-/Status-Chips, getrenntem Auswahlbereich, direkten Da/Nicht-da-Aktionen und zeilenlokalem Save-Feedback umgestellt. Die Summary ist ein kompakter tabellarischer `dl`-Strip, Filter exponieren ihren Pressed-State, und Detail bleibt das bestehende zugaengliche Sheet. Figma-Frames und synthetische responsive Evidence liegen fuer 375/393/834/1194 in Light/Field Mode vor; authentifizierte App-Screenshot-Akzeptanz bleibt mangels Laufzeit-Credentials offen. Public/Kiosk, Reset, Legende, Mitnahmen und Notizen bleiben sekundaer erreichbar. |
...
- Redesign-v2 ist bis einschliesslich R8 code- und funktionsseitig umgesetzt. R7D migriert Spieler auf `AthleteRow`, eine dominante Anlagehandlung, priorisierte Text-/Glyph-Status, einen semantischen Profil-Metrikstrip sowie Detail-Sheet unter 840 px und Detail-Pane ab 840 px. Figma-, maschinenlokale Light-/Field- und gemessene Kontrast-Evidence liegt im R7D-Audit; der generische Signed-in-Visual-Lauf wurde mangels Laufzeit-Credentials geskippt und ist keine Beta-Freigabe. Die gemeinsame R7-Definition-of-Done bleibt nur wegen dokumentierter historischer Screenshot-Evidence-Luecken bewusst offen. Die bestehenden R8-Evidence-Grenzen bleiben unveraendert. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. R7D und der zuvor beauftragte R8 sind abgeschlossen. Als naechster offener Roadmap-Sprint bleibt R9; Scope und Reihenfolge vor Beauftragung gegen den erreichten Stand pruefen.
...
|---|---|---|
| Remote-Supabase-Auth-Settings koennen von lokalen Beta-Defaults abweichen. | Der lokale Audit ist gruen; `qa:beta` ist mangels Laufzeit-Credentials/Testzustand noch offen. Das gehostete Dashboard muss vor externer Beta dieselbe kontrollierte Account-Strategie abbilden. | Echte `qa:beta`-Evidence nach sicherer Credential-Bereitstellung erzeugen und Remote-Dashboard manuell pruefen. |
