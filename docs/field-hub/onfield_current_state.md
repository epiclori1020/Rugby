...
| Heute | Redesign-v2 R5 ist im Code umgesetzt und am 2026-07-10 fachlich gehaertet: `Heute` ist als `Squad heute` Leit-Screen gebaut, mit OnField-Dot-Signature, einem screen-eigenen Header/Sync-Status, Scoreboard-Strip, genau einer dominanten Check-in-Primaerhandlung, severity-sortierter `Aufpassen zuerst`-Athletenliste und sekundaerem Kontext. Kader ist der erwartete aktive Kader plus unerwartet Anwesende; ohne Erwartungsbasis greift der dokumentierte aktive First-run-Fallback. Gelb/Rot spiegeln die hoechste aggregierte Aufmerksamkeit pro eingechecktem Spieler, Returner nur explizites `ja`; alle Gruende bleiben in derselben Zeile sichtbar. Seit R9 landen ausgeloggte Coaches auf `#/welcome`; der signed-in No-Roster-Zustand bleibt ein kompakter operativer Empty State ohne Brand-Hero oder 0-Kachelwand. |
| Designsystem | Sprint 5 hat das erste Core Component Kit in Code, Repo-Dokumentation und Figma-Sheet angelegt. Sprint 25 hat priorisierte Action-Stellen auf bessere Disabled-/Loading-Semantik und `--of-*`-nahe Patterns gehaertet. Redesign-v2 R1A/R1B/R2 haben Field Mode, Typografie-/Weight-/Numeric-Tokens, Theme-Persistenz und den screen-weiten Typo-Refactor umgesetzt. R9 aktiviert selbst gehostetes `Barlow Semi Condensed` 800 fuer Wortmarke/erlaubte Display-Headlines, die gemeinsame Mixed-Case-`OnField•`-Wortmarke sowie explizite `BrandSurface`-Artwork-Modi `hero/texture/none`; Systemfont bleibt operativer Body-Default, 850/900 bleiben verboten. R10 ergaenzt gemeinsame Inline-/Surface-Zustaende, 300-ms-Skeleton-Delay und Reduced-Motion-sichere Sheet-Bewegung. |
| PWA/Install | Sprint 7-26 haben Manifest/Icons, Offline-App-Shell, Install-/Display-Mode-Hinweise sowie harte iPhone/iPad-/Medium-QA aufgebaut. R9 veredelt die Browser-Install-Flaeche mit Field-Ledger-Hero, einer direkten Install-Aktion und denselben nummerierten Safari-Schritten fuer iPhone/iPad; Standalone bleibt kompakt. Manifest, Icons und nativer Splash bleiben bewusst unveraendert, Workbox precacht nun auch die selbst gehosteten Display-WOFF-/WOFF2-Dateien. |
...
| Check-in | Sprint 9 ist umgesetzt; Redesign-v2 R7A hat den Roster am 2026-07-10 auf die gemeinsame v2-`AthleteRow` mit Readiness-Dot, Traffic-/Status-Chips, getrenntem Auswahlbereich, direkten Da/Nicht-da-Aktionen und zeilenlokalem Save-Feedback umgestellt. Die Summary ist ein kompakter tabellarischer `dl`-Strip, Filter exponieren ihren Pressed-State, und Detail bleibt das bestehende zugaengliche Sheet. Figma-Frames und synthetische responsive Evidence liegen fuer 375/393/834/1194 in Light/Field Mode vor; R9 hat den authentifizierten Screen read-only in der echten AppShell-Matrix geprueft, ohne personenbezogene Screenshots zu persistieren. Public/Kiosk, Reset, Legende, Mitnahmen und Notizen bleiben sekundaer erreichbar. |
| Training | Sprint 10 ist umgesetzt; Redesign-v2 R7B hat Training am 2026-07-10 auf gemeinsame v2-`AthleteRow`s und verdichtete Live-Block-Aktionen umgestellt. Die aktuelle Phase bleibt sofort sichtbar; `Erledigt` ist direkt, weitere Status liegen unter `Block anpassen`, und die Navigation ist deutsch. Die Athletenliste zeigt Readiness, priorisierten Coaching-Hinweis, Traffic, Anwesenheit sowie relevante Varianten-/Cap-/Warn-/Sync-Zustaende als Form/Glyph/Text/Farbe und oeffnet weiter das bestehende Detail-Sheet. Preview-Einträge bleiben offen, Filter berücksichtigen meaningful Check-ins, Block-Saves sind gegen konkurrierende Klicks gesperrt und der Sheet-Fokus kehrt zur Row zurück. Loading/Empty/Filter verwenden gemeinsame Patterns; Exposures, Capture, Timeline, Mapping und Varianten bleiben sekundaer erreichbar. Figma-, synthetische und authentifizierte read-only Evidence liegen fuer 375/393/834/1194 in Light/Field Mode vor. |
| Nachbereitung | Sprint 11 ist umgesetzt; Redesign-v2 R7C hat am 2026-07-11 die KPI-Kacheln durch einen kompakten semantischen Summary-Strip ersetzt und die bestehende `TaskQueueRow`-Queue als iPhone-Sheet beziehungsweise iPad/Desktop-Liste-plus-sticky-Pane ausgearbeitet. Pflichtaufgaben, Dauer, genau eine Abschluss-Primaerhandlung, Save-Feedback und die sekundaeren Bereiche fuer Exposures, Coach Review, Metrics, Exercise, Mini-Baseline und Spielerdetails bleiben erhalten. |
| Returner | Redesign-v2 R7C nutzt gemeinsame `AthleteRow`s mit Form/Glyph/Text-Status, eine direkt bedienbare Stufensteuerung und vier strukturierte Cap-Zeilen fuer Speed, COD/Decel, Conditioning und Kontakt. R10 ergaenzt delayed First-Load-Skeleton ohne falschen Clear-Flash, gemeinsamen recoverable Error State und Race-Guard, ohne das vorhandene String-Datenmodell neu zu bauen; Plan/Ist, Reaktion, Entscheidung, Verlauf, Safety, iPhone-Sheet und iPad-Pane bleiben fachlich gleichwertig. Figma- und responsive Evidence liegt fuer 375/393/834/1194 in Light/Field Mode vor; authentifizierte personenbezogene Screenshots werden bewusst nicht persistiert. Die persistierte befuellte Returner-Appmatrix bleibt als historische Evidence-Luecke offen. |
| Spieler | Sprint 12 ist umgesetzt; Redesign-v2 R7D hat den Bereich am 2026-07-13 auf gemeinsame `AthleteRow`s, eine dominante Anlagehandlung und priorisierte Text-/Glyph-Status umgestellt. Der Profilkopf nutzt einen semantischen Metrikstrip fuer letzte Teilnahme, Readiness, Beschwerden und offene Themen. Unter 840 px bleibt das Profil ein fokussiertes Sheet, ab 840 px eine Liste/Panes-Kombination; tiefe Profil-Tabs und Datenmodell bleiben unveraendert. Figma- und maschinenlokale responsive Evidence liegt fuer 375/393/834/1194 in Light/Field Mode vor; die befuellte Element-Kontrastmatrix ist ohne Unterschreitung von 4.5:1 gruen, und R9 hat die authentifizierte befuellte App read-only ohne persistierte Personenscreenshots geprueft. |
| Analyse | Sprint 13 ist umgesetzt; Redesign-v2 R8 hat Analyse am 2026-07-12 responsive und strukturell geschaerft. Compact nutzt ein Filter-Sheet mit getrenntem Entwurf/Anwenden, Medium/Expanded den sichtbaren Filter-Arbeitsraum. R10 ergaenzt zwei leichte zugaengliche React/SVG-Diagramme fuer Wochenbelastung und Belastungsarten, delayed Skeleton sowie recoverable Error State; die semantische responsive Ergebnisliste bleibt die exakte Datenreferenz. Live-Erfassung, Check-in- und Trainingsaktionen bleiben ausserhalb der Analyse. |
| Mehr | Sprint 14 ist umgesetzt; Redesign-v2 R8 hat Bibliothek auf Compact list-first mit Detail-Sheet und ab Medium Liste/Panels gebracht. R10 macht das JSON-Vollbackup zur sichtbaren dominanten Handlung, gruppiert CSV-Tabellen standardmaessig geschlossen und haelt Import im separaten Sheet; Summary-Loading und Fehler nutzen gemeinsame Zustaende. Einstellungen sind ein geordneter Utility-Workspace mit Sync zuerst, gemeinsamem Error State und genau einer zustandsabhaengigen Primaerhandlung. Die Mehr-Subnavigation nutzt Compact 2x2 und ab Medium vier Spalten. |
| Sync/Offline | Sprint 17 ist umgesetzt: Die App-Shell zeigt signed-in global einen kompakten Sync-Status mit Detailsheet, einheitlicher coachnaher Sprache, manueller Sync-Aktion, Backup-Hinweis und Public/Kiosk-Check-in-Detailgruppen. Background Sync bleibt PWA-best-effort und setzt iOS/Safari Background Sync nicht voraus. Redesign-v2 R4 ergaenzt ein gemeinsames Action-Feedback-Muster fuer operative Saves: `gespeichert`, `wartet auf Sync`, `offline lokal gespeichert` und `nicht gespeichert - erneut versuchen` inklusive haptischem Feedback, statt stumm oder roh zu reagieren. |
| Kiosk/Public | Sprint 15 ist umgesetzt: Public/Kiosk nutzen einen eigenen reduzierten Self-Check-in mit linearer Schrittfolge, Review vor Absenden, Abschlusszustand, Kiosk-Auto-Reset und ohne Coach-Notizen/Historie/Analyse. Public speichert keine lokale `submitted`-Marke mehr. Redesign-v2 R3 ist lokal umgesetzt: Kiosk-Exit nutzt ein 3-Sekunden-Hold-only Schloss ohne PIN/Reauth, Coach-Routen bleiben bei aktivem Kiosk-Lock fail-closed gesperrt, und Check-in wird bei fehlender Coach-Session sichtbar blockiert. Redesign-v2 R4/R9 halten Trust-Copy ruhig und zeigen Texture-Artwork nur im initialen Namensschritt. Das echte R9-`qa:beta` ist mit kontrollierter R5-/Kiosk-Fixture, `finally`-Cleanup und verifizierter Remote-Abwesenheit gruen. |
| Brand-Surfaces | Redesign-v2 R9 ist umgesetzt: Field Ledger liegt als komprimiertes Light-/Field-WebP im Repo, `BrandSurface` ist mit sicherem `none`-Default auf `hero/texture/none` begrenzt, `#/welcome` fuehrt Login/First Run in einer zusammenhaengenden Surface, Public/Kiosk verlieren die Textur ab Readiness und Live-Coaching bleibt ohne Hero. Wortmarke, Display-Font, primaere Zustandsframes und der Spec-Closeout sind im Figma-Board `Redesign v2 R9 Brand Surfaces` gespiegelt; die neutrale Welcome-Browsermatrix deckt 375/393/744/834/1194 in Light/Field Mode ab. |
...
- R7A Check-in Rows Evidence: `docs/field-hub/2026-07-10_onfield_r7a_checkin_rows_audit.md`
- R7B Training Live Rows Evidence: `docs/field-hub/2026-07-10_onfield_r7b_training_live_rows_audit.md`
- R7C Nachbereitung & Returner Rows Evidence: `docs/field-hub/2026-07-11_onfield_r7c_post_session_returner_rows_audit.md`
- R8 Responsive, Field Mode, Analyse & Mehr Evidence: `docs/field-hub/2026-07-12_onfield_r8_responsive_field_mode_analysis_more_audit.md`
- R9 Brand-Surfaces, Wortmarke & Display-Font Evidence: `docs/field-hub/2026-07-13_onfield_r9_brand_surfaces_wordmark_display_font_audit.md`
...
- R7D Spielerliste & Profilkopf Evidence: `docs/field-hub/2026-07-13_onfield_r7d_player_list_profile_header_audit.md`.
...
- Redesign-v2 R1-R10 ist code- und funktionsseitig abgeschlossen und seit 2026-07-14 auf `https://field-hub-beta.vercel.app` produktiv deployed. R10 finalisiert gemeinsame States/Skeletons, native Analysecharts, Backup-/CSV-Hierarchie, destruktives In-App-Sheet, Skalen-Anker, coach-sichere Fehlercopy und reduzierte Motion/Haptik. `qa:local` ist mit 111 Testdateien / 754 Tests, Build, PWA- und generischer Visual-QA-Matrix gruen; der authentifizierte read-only Audit pruefte Analyse, Export, Returner und das Delete-Sheet auf iPhone-/iPad-Breiten. Das vollstaendige historische Launch-Evidence-Gate bleibt wegen der dokumentierten R5-/R7- und persistierten authentifizierten Screenshot-Luecken teilweise offen. `qa:beta` wurde ohne ausdrueckliches Remote-Mutations-Opt-in nicht als Erfolg gewertet.

## Naechste empfohlene Schritte

1. Vor externer Beta eine echte `qa:beta`-Ausfuehrung mit ausdruecklichem Opt-in, frischen QA-Credentials und kontrolliertem Fixture-Cleanup durchfuehren; ein Skip bleibt kein Beta-Gate.
2. Physische iPhone-/iPad-Safari-Safe-Area sowie die noch fehlende persistierbare synthetische Kernscreen-/historische Vorher-Nachher-Matrix schliessen, ohne personenbezogene Screenshots zu persistieren.
3. Weitere Export-IA, Motion oder externe Brand-Assets nur nach Coach-/Launch-Evidence als eigener Scope planen; keine implizite Fortsetzung der abgeschlossenen R1-R10-Roadmap.

## Offene Risiken

| Risiko | Auswirkung | Naechster Schritt |
|---|---|---|
| Remote-Supabase-Auth-Settings koennen von lokalen Beta-Defaults abweichen. | Lokaler Audit und echtes R9-`qa:beta` sind gruen; ein erfolgreicher Bestandsaccount-Login beweist aber nicht jede Dashboard-Signup-Einstellung. | Remote-Dashboard vor externer Beta weiterhin manuell gegen die kontrollierte Account-Strategie pruefen. |
...
| Core Components sind noch nicht in jedem Utility-/Analysebereich ausgerollt. | R7A-R7D nutzen die gemeinsamen Row-/Status-/Sheet-Muster nun in allen beauftragten Kernflows einschliesslich Spieler; weitere Migrationen bleiben absichtlich ausserhalb R7D. | Nur in einem kuenftig beauftragten Sprint gezielt migrieren, nicht nebenbei. |
