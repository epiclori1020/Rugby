...
| Heute | Redesign-v2 R5 ist umgesetzt: `Heute` ist als `Squad heute` Leit-Screen gebaut, mit OnField-Dot-Signature, Scoreboard-Strip, genau einer dominanten Check-in-Primaerhandlung, severity-sortierter `Aufpassen zuerst`-Athletenliste und sekundaerem Kontext im Accordion. First-run/signed-out bleibt ein kompakter operativer Empty State ohne Brand-Hero oder 0-Kachelwand. |
