...
- Redesign-v2 ist bis R7A code- und Funktionsseitig umgesetzt: R1A/R1B Tokens, Theme und Field Mode; R2 Typo-/Weight-/Numeric-Refactor; R3 Kiosk-Schloss; R4 Trust/Save-Feedback; R5 `Squad heute`; R6 Einheit-Cockpit/Returner-Loop; R7A Check-in Rows und gemeinsame Row-/Status-Primitives. R7A ersetzt die eigene Check-in-Legacy-Row durch `AthleteRow`, Readiness-Dot und Glyph-plus-Text-Chips, behaelt beide direkten Anwesenheitsaktionen sowie das Detail-Sheet und ist mit Figma-/synthetischer Viewport-Evidence, authentifizierter read-only Live-QA und `qa:local` geprueft. Die historische Vorher-Screenshot-Evidence fehlt weiterhin; die authentifizierte Live-Matrix wurde aus Datenschutzgruenden nicht mit echten Spielernamen als Screenshot gespeichert. R7A ist deshalb nicht als vollstaendig dokumentiertes Vorher/Nachher-Screenshot-Gate zu bezeichnen. `qa:beta` bleibt ein separates, nicht ausgefuehrtes Beta-Gate. R7B-R7D wurden nicht begonnen. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. Vor einem explizit beauftragten R7B-Slice den offenen R7A-Evidence-Status sichtbar lassen oder bewusst als akzeptierte Dokumentationsluecke entscheiden; R7B `Training Live Rows` bleibt der naechste Abhaengigkeitsschritt. R7C/R7D nicht vor R7B nebenbei umsetzen.
...
| Core Components sind noch nicht screen-weit ausgerollt. | R7A stabilisiert AthleteRow/Status fuer Check-in; Training, Nachbereitung/Returner und Spieler nutzen teilweise weiter Legacy-Klassen und Inline-Muster. | Nur in den beauftragten R7B-, R7C- und R7D-Slices gezielt migrieren. |
