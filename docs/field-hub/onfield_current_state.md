...
- Redesign-v2 ist bis R7C code- und Funktionsseitig umgesetzt: R1A/R1B Tokens, Theme und Field Mode; R2 Typo-/Weight-/Numeric-Refactor; R3 Kiosk-Schloss; R4 Trust/Save-Feedback; R5 `Squad heute`; R6 Einheit-Cockpit/Returner-Loop; R7A Check-in Rows; R7B Training Live Rows; R7C Nachbereitung & Returner Rows. R7C nutzt `TaskQueueRow`, `AthleteRow`, strukturierte Stufen-/Cap-Controls und Sheet/Pane-Paritaet ohne Datenumbau. Figma-Matrix fuer 375/393/834/1194 in Light/Field Mode, authentifizierte read-only Browserpruefung und `qa:local` sind gruen; die persistierte React-Screenshotmatrix mit befuellter Returner-Row bleibt als Evidence-Luecke offen. Der generische Sprint-19-Signed-in-Teil wurde mangels Env-Credentials geskippt und gilt nicht als Beta-Gate; die separate authentifizierte R7C-Pruefung lief erfolgreich. `qa:beta` bleibt separat offen. Historische Vorher-Screenshot-Luecken werden nicht nachtraeglich gestellt. R7D wurde nicht begonnen. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. R7D `Spielerliste & Profilkopf` ist der naechste Abhaengigkeitsschritt. R8 nicht vor R7D nebenbei umsetzen; die offenen historischen Vorher-Screenshot-Luecken sichtbar lassen.
