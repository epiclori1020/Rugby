...
- Redesign-v2 ist gestartet: R1A Tokens v2 & Field-Mode Token-Fundament ist abgeschlossen. R1B Theme-Persistenz & Field-Mode Toggle ist abgeschlossen: Field Mode ist ueber Einstellungen steuerbar und persistiert `system/light/dark`, ohne R5/R8/R9 vorzuziehen. R2 Typo-/Weight-/Numeric-Refactor ist abgeschlossen: zentrale Redesign-CSS-Dateien nutzen Typo-/Weight-Tokens statt roher Font-Werte, 850/900-Gewichte sind in diesen Dateien verboten, und relevante Zahlenwerte nutzen tabular numerals. R3 Kiosk-Schloss ist lokal umgesetzt: Der Exit aus dem Kiosk ist ein 3-Sekunden-Hold-only Coach-Modus-Intent, keine Auth-Garantie; Coach-Routen bleiben bei aktivem Lock fail-closed gesperrt. R4 Trust, Save-Feedback & Roh-Copy ist umgesetzt: Training, Nachbereitung und Returner zeigen gemeinsames Save-Feedback, Auth/Public/Kiosk mappen rohe Fehler auf coachnahe Meldungen. R5 `Squad heute` ist im Code umgesetzt und fachlich gehaertet: gemeinsame Domain-Ableitung fuer Kader/Anwesenheit/Aufmerksamkeit, eine Zeile pro anwesendem Spieler mit sichtbaren Gruenden, ein Header/Sync-Status, R5-spezifische Responsive-Regeln, gehaerteter authentifizierter E2E-Gate und abgeglichene Figma-Frames fuer 375/393/834/1194. Die R5-Redesign-Akzeptanz und `qa:beta` bleiben offen, bis echte Beta-Credentials, der definierte Datenzustand, Remote-Mutation-Opt-in und authentifizierte Light-/Field-Mode-Evidence vorhanden sind. R6 `Einheit-Cockpit & Returner im Loop` ist abgeschlossen: `#/unit/returners` teilt den Session-Kontext mit Check-in, Training und Nachbereitung, kontextuelle Einstiege kehren stabil zum Ursprung zurueck, der neutrale Returner-Zustand vermeidet Dauer-Alarm, Training nutzt Overflow fuer Sekundaeres und Nachbereitung genau einen sticky Closeout. Ein authentifizierter R6-Gate prueft 40 Viewport-/Theme-/Route-Kombinationen; `#/more/returners` bleibt Backup. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. Redesign-v2 nur nach explizitem Auftrag mit dem naechsten Roadmap-Slice fortsetzen; nach R6 ist R7A `Check-in Rows & shared row primitives` der naechste Abhaengigkeitsschritt. R7B-R7D nicht vor R7A vorziehen.
