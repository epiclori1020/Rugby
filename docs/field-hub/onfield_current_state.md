...
- Redesign-v2 ist bis R7C sowie im explizit beauftragten R8 code- und funktionsseitig umgesetzt: R1A/R1B Tokens, Theme und Field Mode; R2 Typo-/Weight-/Numeric-Refactor; R3 Kiosk-Schloss; R4 Trust/Save-Feedback; R5 `Squad heute`; R6 Einheit-Cockpit/Returner-Loop; R7A Check-in Rows; R7B Training Live Rows; R7C Nachbereitung & Returner Rows; R8 Responsive, Field-Mode-QA und Analyse/Mehr. R8 liefert strukturelle Compact/Medium/Expanded-Vertraege, responsive und screenreader-beschriftete Analyseergebnisse, `MetricTile`, Bibliothek Sheet/Pane ohne verschachtelte Modals, Export-/Settings-Hierarchie, 110 Coach-, 40 authentifizierte R8- und vier Kiosk-Welcome-Browserkombinationen in Light/Field Mode sowie einen gruenen kompletten Kiosk-Submit/Cleanup-Smoke. Dark-Primary erreicht 7,82:1; sichtbare Text-/Control-Kontraste, Content-Clearance und Sticky-Kollisionen werden automatisiert geprueft. `qa:local` ist gruen. `qa:beta` wurde nicht als Erfolg gewertet, sondern blockiert am fehlenden vorgelagerten R5-Kader-/Check-in-/Aufmerksamkeitszustand. Offene R8-Evidence-Grenzen sind ein echter iOS-Safe-Area-Lauf mit nicht-null Insets, eine persistierte synthetisch befuellte Analyse-App-Matrix und ein gueltiger Public-Form-/Review-/Success-Remote-Lauf; die vorhandene Figma-/Komponenten-/Browser-Evidence ist im R8-Audit abgegrenzt. R7D wurde auf ausdruecklichen Nutzerscope nicht vorgezogen und bleibt offen. Historische Evidence-Luecken werden nicht nachtraeglich gestellt. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. R7D `Spielerliste & Profilkopf` bleibt der offene Abhaengigkeitsschritt und wurde in R8 nicht mitgezogen. Danach die Roadmap-Reihenfolge neu gegen den dann erreichten Stand pruefen.
...
| Core Components sind noch nicht screen-weit ausgerollt. | R7A-R7C stabilisieren AthleteRow/TaskQueueRow/Status/Sheet fuer Check-in, Training, Nachbereitung und Returner; Spieler nutzt teilweise weiter Legacy-Klassen und Inline-Muster. | Nur im beauftragten R7D-Slice gezielt migrieren. |
