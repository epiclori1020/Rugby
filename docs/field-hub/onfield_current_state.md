...
| Returner | Redesign-v2 R7C nutzt gemeinsame `AthleteRow`s mit Form/Glyph/Text-Status, eine direkt bedienbare Stufensteuerung und vier strukturierte Cap-Zeilen fuer Speed, COD/Decel, Conditioning und Kontakt. Das vorhandene String-Datenmodell bleibt bewusst bestehen; Plan/Ist, Reaktion, Entscheidung, Verlauf, Safety, iPhone-Sheet und iPad-Pane bleiben fachlich gleichwertig. Figma- und responsive Evidence liegt fuer 375/393/834/1194 in Light/Field Mode vor; die authentifizierte read-only Pruefung enthielt im gewaehlten Kontext keine populated Returner-Row. Eine persistierte React-Screenshotmatrix mit befuellter Returner-Row bleibt daher als visuelle Evidence-Luecke offen. |
...
| Analyse | Sprint 13 ist umgesetzt; Redesign-v2 R8 hat Analyse am 2026-07-12 responsive und strukturell geschaerft. Compact nutzt ein Filter-Sheet mit getrenntem Entwurf/Anwenden, Medium/Expanded den sichtbaren Filter-Arbeitsraum. HTML-Tabellen wurden durch eine semantische responsive Ergebnisliste ersetzt; Kennzahlen nutzen das gemeinsame `MetricTile`-Primitive und tabellarische Ziffern. Live-Erfassung, Check-in- und Trainingsaktionen bleiben ausserhalb der Analyse. |
| Mehr | Sprint 14 ist umgesetzt; Redesign-v2 R8 hat Bibliothek auf Compact list-first mit Detail-Sheet und ab Medium Liste/Panels gebracht. Export & Backup besitzt einen Summary-Strip, eine dominante JSON-Backup-Handlung, CSV-Rows und einen Import-Sheet. Einstellungen sind ein geordneter Utility-Workspace mit Sync zuerst und genau einer zustandsabhaengigen Primaerhandlung. Die Mehr-Subnavigation nutzt Compact 2x2 und ab Medium vier Spalten. |
...
- Redesign-v2 ist bis einschliesslich R9 code- und funktionsseitig umgesetzt. R9 finalisiert Wortmarke/Dot-Signature, Display-Font-Test, Field-Ledger-Rasterassets, Welcome/Auth/Install und die erlaubten Public-/Kiosk-Einstiege in Light/Field Mode. Das Figma-Board und der R9-Audit enthalten Integrity- und Vorher/Nachher-Evidence. Die gemeinsamen historischen R7-Evidence-Grenzen und die R8-Grenzen bleiben unveraendert. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. R9 ist abgeschlossen. Als naechster offener Roadmap-Sprint bleibt R10; ihn nur separat beauftragt und ohne Rueckvermischung in R9 umsetzen.
