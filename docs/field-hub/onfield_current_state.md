...
- Redesign-v2 ist bis R7B code- und Funktionsseitig umgesetzt: R1A/R1B Tokens, Theme und Field Mode; R2 Typo-/Weight-/Numeric-Refactor; R3 Kiosk-Schloss; R4 Trust/Save-Feedback; R5 `Squad heute`; R6 Einheit-Cockpit/Returner-Loop; R7A Check-in Rows; R7B Training Live Rows. R7B nutzt die gemeinsamen Row-/Status-Primitives, behaelt Detail-/Quick-Action-/Save-Verhalten und verdichtet den Live-Block auf eine direkte Abschlussaktion plus Anpassungs-Overflow. Figma-, synthetische und authentifizierte read-only Browsermatrix fuer 375/393/834/1194 in Light/Field Mode sowie `qa:local` sind gruen. Der generische Sprint-19-Signed-in-Teil wurde mangels Env-Credentials geskippt und gilt nicht als Beta-Gate; die separate authentifizierte R7B-Prüfung lief erfolgreich. `qa:beta` bleibt separat offen. Die historischen Vorher-Screenshot-Luecken von R7A/R7B sind dokumentiert und wurden nicht nachtraeglich gestellt. R7C/R7D wurden nicht begonnen. `docs/superpowers/plans/2026-07-08-onfield-redesign-v2-heritage-field-instrument.md` bleibt die aktive Roadmap fuer Route A "Heritage Field Instrument".

## Naechste empfohlene Schritte

1. R7C `Nachbereitung & Returner Rows` ist der naechste Abhaengigkeitsschritt. R7D nicht vor R7C nebenbei umsetzen; die offenen historischen Vorher-Screenshot-Luecken von R7A/R7B sichtbar lassen.
...
| Core Components sind noch nicht screen-weit ausgerollt. | R7A/R7B stabilisieren AthleteRow/Status fuer Check-in und Training; Nachbereitung/Returner und Spieler nutzen teilweise weiter Legacy-Klassen und Inline-Muster. | Nur in den beauftragten R7C- und R7D-Slices gezielt migrieren. |
