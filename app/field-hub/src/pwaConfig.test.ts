...
describe('Sprint 10 PWA configuration', () => {
  ...
  it('uses PNG install icons for iOS and PWA manifests', () => {
    ...
    expect(existsSync(join(projectRoot, 'public/pwa-maskable-192x192.png'))).toBe(true)
    expect(existsSync(join(projectRoot, 'public/pwa-maskable-512x512.png'))).toBe(true)
    ...
    expect(readPngDimensions('pwa-maskable-192x192.png')).toEqual({ width: 192, height: 192 })
    expect(readPngDimensions('pwa-maskable-512x512.png')).toEqual({ width: 512, height: 512 })
